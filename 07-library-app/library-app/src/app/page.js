    ...
    <main className="min-h-screen bg-neutral-950 text-neutral-100">
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <ReservationForm
          ...
          onReservationCreated={(finalReservation) => {
            setReservations((prev) =>
              prev.map((r) =>
                r.id === finalReservation.id ? finalReservation : r
              )
            );
            addToast("Rezerwacja potwierdzona");
          }}
          onAddOptimistic={(optimisticReservation) => {
            setReservations((prev) => [optimisticReservation, ...prev]);

            if (addOptimisticReservationRef.current) {
              addOptimisticReservationRef.current(optimisticReservation);
            }
          }}
          onServerError={(tempId) => {
            setReservations((prev) => prev.filter((r) => r.id !== tempId));
            addToast("Błąd podczas rezerwacji", "error");
          }}
        />
        ...
        <ReservationList
          ...
          registerAddOptimistic={(fn) => {
            addOptimisticReservationRef.current = fn;
          }}
        />
